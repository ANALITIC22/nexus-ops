...
// ── Actualizar badges del sidebar según matrices propias ─────
function _updateNavBadges() {
  const uid         = AppState.adminSession?.uid;
  const isSuperAdmin = AppState.adminSession?.isSuperAdmin;

  // SuperAdmin: cuenta todo. Admin normal: solo sus matrices.
  const misMats = isSuperAdmin
    ? AppState.matrices
    : (uid ? AppState.matrices.filter(m => m.creadoPor === uid) : AppState.matrices);

  // IDs de matrices para filtrar objetivos y tareas
  const misMatIds = new Set(misMats.map(m => m.id));

  const misObj    = isSuperAdmin
    ? AppState.objetivos
    : AppState.objetivos.filter(o => misMatIds.has(o.matrizId));
  const misTareas = isSuperAdmin
    ? AppState.tareas
    : AppState.tareas.filter(t => misMatIds.has(t.matrizId));
  ...
  set('navMatricesBadge',  misMats.length);
  set('navObjetivosBadge', misObj.length);
  set('navTareasBadge',    misTareas.length);
}

// Exportar para llamarla externamente cuando cambia la sesión
export { _updateNavBadges as updateNavBadges };
